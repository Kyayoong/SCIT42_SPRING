...
public class FileService {
	...
	public static String saveFile(MultipartFile mfile, String uploadPath) {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd"); //date의 형식을 정함
		String savedFilename = sdf.format(new Date());			 //정해진 형식으로 저장함.
		...
		while (true) {				// c:/upload/20220802.jpg
			serverFile = new File(uploadPath + "/" + savedFilename + ext); //경로 생성 (저장경로/새로저장된파일이름+확장자)
			//같은 이름의 파일이 없으면 나감.
			if ( !serverFile.isFile()) break;	// isfile : true일 경우 그 경로에 같은 이름의 파일이 있다.
		...
		//파일 저장
		try {
			mfile.transferTo(serverFile); //기존의 스트림 생성 후, 반복문으로 옮기는 과정을 메소드로 제공해줌.
		} catch (Exception e) {
			savedFilename = null;
			e.printStackTrace();
		}
